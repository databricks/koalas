...
class KoalasBoxPlot(BoxPlot):
    ...
    def _compute_stats(data, colname, whis, precision):
        ...
        pdf = data._kdf._sdf.agg(
            *[
                F.expr(
                    "approx_percentile({}, {}, {})".format(
                        colname, q, int(1.0 / precision)
                    )
        ...
        pdf.loc[:, "{}_lfence".format(colname)] = (
            pdf.loc[:, p25] - whis * pdf.loc[:, iqr]
        )
        pdf.loc[:, "{}_ufence".format(colname)] = (
            pdf.loc[:, p75] + whis * pdf.loc[:, iqr]
        )
...
class KoalasHistPlot(HistPlot):
    ...
    def _compute_hist(sdf, bins):
        ...
        indexes = pd.DataFrame(
            {bucket_name: np.arange(0, len(bins) - 1), "bucket": bins[:-1]}
        )
    ...
    def _get_ind(self, y):
        ...
        if self.ind is None:
            min_val, max_val = y.select(
                F.min(y.columns[-1]), F.max(y.columns[-1])
            ).first()

            sample_range = max_val - min_val
            ind = np.linspace(
                min_val - 0.5 * sample_range, max_val + 0.5 * sample_range, 1000,
            )
            ...
            ind = np.linspace(
                min_val - 0.5 * sample_range, max_val + 0.5 * sample_range, self.ind,
            )
        else:
            ind = self.ind
        return ind

...
class StatsTest(ReusedSQLTestCase, SQLTestUtils):
    def _test_stat_functions(self, pdf, kdf):
        ...
        for funcname in functions:
            self.assert_eq(
                getattr(kdf.A, funcname)(), getattr(pdf.A, funcname)(), almost=True
            )
            self.assert_eq(
                getattr(kdf, funcname)(), getattr(pdf, funcname)(), almost=True
            )
    ...
    def test_some_stats_functions_should_discard_non_numeric_columns_by_default(self):
        pdf = pd.DataFrame(
            {"i": [0, 1, 2], "b": [False, False, True], "s": ["x", "y", "z"]}
        )
        kdf = ks.from_pandas(pdf)
    ...
    def test_stats_on_non_numeric_columns_should_be_discarded_if_numeric_only_is_true(
        ...
        self.assertEqual(
            len(kdf.sum(numeric_only=True)), len(pdf.sum(numeric_only=True))
        )
        self.assertEqual(
            len(kdf.mean(numeric_only=True)), len(pdf.mean(numeric_only=True))
        )

        self.assertEqual(
            len(kdf.var(numeric_only=True)), len(pdf.var(numeric_only=True))
        )
        self.assertEqual(
            len(kdf.std(numeric_only=True)), len(pdf.std(numeric_only=True))
        )

        self.assertEqual(
            len(kdf.kurtosis(numeric_only=True)), len(pdf.kurtosis(numeric_only=True))
        )
        self.assertEqual(
            len(kdf.skew(numeric_only=True)), len(pdf.skew(numeric_only=True))
        )

    def test_stats_on_non_numeric_columns_should_not_be_discarded_if_numeric_only_is_false(
        self,
    ):
        pdf = pd.DataFrame(
            {"i": [0, 1, 2], "b": [False, False, True], "s": ["x", "y", "z"]}
        ...
        self.assertEqual(
            len(kdf.sum(numeric_only=False)), len(pdf.sum(numeric_only=False))
        )

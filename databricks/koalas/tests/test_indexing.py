...
class BasicIndexingTest(ComparisonTestBase):
    ...
    def test_indexing(self, df):
        ...
        self.assertRaisesRegex(
            KeyError, "unknown", lambda: df.set_index(["month", "unknown"])
        )
        ...
        self.assertRaisesRegex(
            KeyError, "unknown.*month", lambda: df1.reset_index(level="unknown")
        )
        self.assertRaisesRegex(
            KeyError,
            "Level unknown not found",
...
class IndexingTest(ReusedSQLTestCase):
    ...
    def test_at(self):
        ...
        with self.assertRaises(
            TypeError, msg="Use DataFrame.at like .at[row_index, column_name]"
        ):
            kdf.at[3]
        with self.assertRaises(
            TypeError, msg="Use DataFrame.at like .at[row_index, column_name]"
    ...
    def test_iat(self):
        ...
        with self.assertRaises(
            TypeError, msg="Use Series.iat like .iat[row_integer_position]"
        ):
            test_series.iat[3, "b"]
    ...
    def test_loc(self):
        ...
        pdf = pd.DataFrame(
            {"a": [1, 2, 3, 4, 5, 6, 7, 8, 9]}, index=[0, 1, 1, 2, 2, 2, 4, 5, 6]
        )
        kdf = ks.from_pandas(pdf)
        ...
        pdf = pd.DataFrame(
            {"a": [1, 2, 3, 4, 5, 6, 7, 8, 9]}, index=[6, 5, 5, 4, 4, 4, 2, 1, 0]
        )
        kdf = ks.from_pandas(pdf)
        ...
        kdf = ks.DataFrame(
            [[1, 2], [4, 5], [7, 8]], index=["cobra", "viper", "sidewinder"]
        )
    ...
    def test_loc2d(self):
        ...
        self.assertRaises(SparkPandasIndexingError, lambda: kdf.a.loc[3:, 3])
        self.assertRaises(
            SparkPandasIndexingError, lambda: kdf.a.loc[kdf.a % 2 == 0, 3]
        )
    ...
    def test_loc_timestamp_str(self):
        ...
        self.assert_eq(
            pdf.loc["2011-01-02":"2011-01-05"], kdf.loc["2011-01-02":"2011-01-05"]
        )
        ...
        self.assert_eq(
            pdf.A.loc["2011-01-02":"2011-01-05"], kdf.A.loc["2011-01-02":"2011-01-05"]
        )
    ...
    def test_iloc(self):
        ...
        for indexer in [0, [0], [0, 1], [1, 0], [False, True, True], slice(0, 1)]:
            ...
            self.assert_eq(kdf.iloc[:-1, indexer], pdf.iloc[:-1, indexer])
            self.assert_eq(
                kdf.iloc[kdf.index == 2, indexer], pdf.iloc[pdf.index == 2, indexer]
            )
    ...
    def test_iloc_multiindex_columns(self):
        ...
        for indexer in [0, [0], [0, 1], [1, 0], [False, True, True, True], slice(0, 1)]:
            ...
            self.assert_eq(kdf.iloc[:-1, indexer], pdf.iloc[:-1, indexer])
            self.assert_eq(
                kdf.iloc[kdf.index == "B", indexer], pdf.iloc[pdf.index == "B", indexer]
            )
    ...
    def test_setitem(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Only a dataframe with one column can be assigned"
        ):
    ...
    def test_iloc_raises(self):
        ...
        with self.assertRaisesRegex(
            SparkPandasIndexingError, "Only accepts pairs of candidates"
        ):
        ...
        with self.assertRaisesRegex(
            TypeError, "cannot do slice indexing with these indexers"
        ):
            kdf.iloc[:"b", :]

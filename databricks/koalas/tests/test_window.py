...
class ExpandingRollingTest(ReusedSQLTestCase, TestUtils):
    def test_missing(self):
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeExpanding, inspect.isfunction
        )
        ...
        for name in deprecated_functions:
            with self.assertRaisesRegex(
                PandasNotImplementedError,
                "method.*Expanding.*{}.*is deprecated".format(name),
            ):
                getattr(kdf.expanding(1), name)()  # Frame
        ...
        # Rolling functions
        missing_functions = inspect.getmembers(
            _MissingPandasLikeRolling, inspect.isfunction
        )
        unsupported_functions = [
            name
            for (name, type_) in missing_functions
        ...
        for name in deprecated_functions:
            with self.assertRaisesRegex(
                PandasNotImplementedError,
                "method.*Rolling.*{}.*is deprecated".format(name),
            ):
                getattr(kdf.rolling(1), name)()  # Frame
            with self.assertRaisesRegex(
                PandasNotImplementedError,
        ...
        for name in deprecated_properties:
            with self.assertRaisesRegex(
                PandasNotImplementedError,
                "property.*Expanding.*{}.*is deprecated".format(name),
            ):
                getattr(kdf.expanding(1), name)  # Frame
            with self.assertRaisesRegex(
                PandasNotImplementedError,
        ...
        for name in deprecated_properties:
            with self.assertRaisesRegex(
                PandasNotImplementedError,
                "property.*Rolling.*{}.*is deprecated".format(name),
            ):
                getattr(kdf.rolling(1), name)()  # Frame
            with self.assertRaisesRegex(
                PandasNotImplementedError,
    ...
    def test_missing_groupby(self):
        ...
        # Rolling functions
        missing_functions = inspect.getmembers(
            _MissingPandasLikeRollingGroupby, inspect.isfunction
        )
        unsupported_functions = [
            name
            for (name, type_) in missing_functions

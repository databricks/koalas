...
class NamespaceTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def test_to_datetime(self):
        ...
        self.assert_eq(
            pd.to_datetime(1490195805, unit="s"), ks.to_datetime(1490195805, unit="s")
        )
        self.assert_eq(
    ...
    def test_concat(self):
        ...
        self.assert_eq(
            ks.concat([kdf, kdf.reset_index()]), pd.concat([pdf, pdf.reset_index()])
        )
        ...
        self.assertRaisesRegex(
            TypeError, "first argument must be", lambda: ks.concat(kdf)
        )
        self.assertRaisesRegex(
        ...
        self.assertRaisesRegex(
            ValueError, "No objects to concatenate", lambda: ks.concat([])
        )
        ...
        self.assert_eq(
            ks.concat([kdf3, kdf3.reset_index()]), pd.concat([pdf3, pdf3.reset_index()])
        )

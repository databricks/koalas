...
class CsvTest(ReusedSQLTestCase, TestUtils):
    ...
    def test_read_csv_with_quotechar(self):
        with self.csv_file(self.q_quoted_csv_text) as fn:
            self.assert_eq(
                ks.read_csv(fn, quotechar="Q"),
                pd.read_csv(fn, quotechar="Q"),
                almost=True,
            )
    ...
    def test_to_csv(self):
        ...
        self.assert_eq(
            kdf.to_csv(columns=["aa"]), pdf.to_csv(columns=["aa"], index=False)
        )
        self.assert_eq(kdf.aa.to_csv(), pdf.aa.to_csv(index=False, header=True))

        pdf = pd.DataFrame(
            {"a": [1, np.nan, 3], "b": ["one", "two", None],}, index=[0, 1, 3]
        )

        kdf = ks.from_pandas(pdf)

        self.assert_eq(
            kdf.to_csv(na_rep="null"), pdf.to_csv(na_rep="null", index=False)
        )
        self.assert_eq(
            kdf.a.to_csv(na_rep="null"),
            pdf.a.to_csv(na_rep="null", index=False, header=True),
        )

        pdf = pd.DataFrame(
            {"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0],}, index=[0, 1, 3]
        )
    ...
    def test_to_csv_with_path(self):
        ...
        output_paths = [
            path for path in os.listdir(self.tmp_dir) if path.startswith("part-")
        ]
    ...
    def test_to_csv_with_path_and_basic_options_multiindex_columns(self):
        ...
        kdf.to_csv(
            self.tmp_dir, num_files=1, sep="|", header=["a"], columns=[("x", "a")]
        )
        pdf.columns = ["a", "b"]

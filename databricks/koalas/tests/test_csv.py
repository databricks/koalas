...
class CsvTest(ReusedSQLTestCase, TestUtils):
    ...
    def test_read_csv(self):
        with self.csv_file(self.csv_text) as fn:
            ...
            self.assertRaisesRegex(
                ValueError, "non-unique", lambda: koalas.read_csv(fn, names=["n", "n"])
            )
            self.assertRaisesRegex(
                ValueError,
            ...
            self.assertRaisesRegex(
                ValueError, "Usecols do not match.*3", lambda: koalas.read_csv(fn, usecols=[1, 3])
            )
            self.assertRaisesRegex(
                ValueError,
            ...
            self.assertRaisesRegex(
                ValueError, "Unknown header argument 1", lambda: koalas.read_csv(fn, header="1")
            )
    ...
    def test_read_csv_with_mangle_dupe_cols(self):
        self.assertRaisesRegex(
            ValueError, "mangle_dupe_cols", lambda: koalas.read_csv("path", mangle_dupe_cols=False)
        )

    def test_read_csv_with_parse_dates(self):
        self.assertRaisesRegex(
            ValueError, "parse_dates", lambda: koalas.read_csv("path", parse_dates=True)
        )

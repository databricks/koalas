...
class GroupByTest(ReusedSQLTestCase, TestUtils):
    def test_groupby(self):
        ...
        self.assert_eq(
            kdf.groupby("a")["a"].sum().sort_index(),
        ...
        self.assert_eq(
            kdf.a.groupby(kdf.b).sum().sort_index(),
        ...
        self.assertRaises(ValueError, lambda: kdf.groupby("a", as_index=False)[["a"]])
        self.assertRaises(
            ValueError, lambda: kdf.groupby("a", as_index=False)[["a", "c"]]
        )
        self.assertRaises(
            ValueError, lambda: kdf.groupby(0, as_index=False)[["a", "c"]]
        )
        self.assertRaises(
            KeyError, lambda: kdf.groupby([0], as_index=False)[["a", "c"]]
        )
    ...
    def test_groupby_multiindex_columns(self):
        ...
        self.assert_eq(
            kdf.groupby(("x", "a")).sum().sort_index(),
    ...
    def test_aggregate(self):
        ...
        for as_index in [True, False]:
            stats_kdf = kdf.groupby("A", as_index=as_index).agg(
                {"B": "min", "C": "sum"}
            )
            stats_pdf = pdf.groupby("A", as_index=as_index).agg(
                {"B": "min", "C": "sum"}
            )
            self.assert_eq(
        ...
        for as_index in [True, False]:
            ...
            self.assert_eq(
                stats_kdf.sort_values(by=[("X", "B"), ("Y", "C")]).reset_index(
                    drop=True
                ),
        ...
        stats_kdf = kdf.groupby(("X", "A")).agg(
            {("X", "B"): ["min", "max"], ("Y", "C"): "sum"}
        )
        stats_pdf = pdf.groupby(("X", "A")).agg(
            {("X", "B"): ["min", "max"], ("Y", "C"): "sum"}
        )
        self.assert_eq(
            stats_kdf.sort_values(
    ...
    def test_describe(self):
        ...
        for data in datas:
            ...
            quantile_pdf = pdf.groupby("a").quantile(
                percentiles, interpolation="nearest"
            )
        ...
        for data in datas:
            ...
            describe_pdf = pdf.groupby("a").describe().sort_index()
            self.assertRaises(
                NotImplementedError, lambda: kdf.groupby("a").describe().sort_index()
            )
    ...
    def test_all_any(self):
        ...
        self.assert_eq(
            kdf.B.groupby(kdf.A).all().sort_index(),
            pdf.B.groupby(pdf.A).all().sort_index(),
        )
        self.assert_eq(
            kdf.B.groupby(kdf.A).any().sort_index(),
    ...
    def test_nunique(self):
        ...
        self.assert_eq(
            kdf.groupby("a").nunique().sort_index(),
    ...
    def test_value_counts(self):
        pdf = pd.DataFrame(
            {"A": [1, 2, 2, 3, 3, 3], "B": [1, 1, 2, 3, 3, 3]}, columns=["A", "B"]
        )
        kdf = ks.from_pandas(pdf)
        self.assert_eq(
    ...
    def test_size(self):
        ...
        kdf = ks.from_pandas(pdf)
        self.assert_eq(
            kdf.groupby("A").size().sort_index(), pdf.groupby("A").size().sort_index()
        )
        self.assert_eq(
            kdf.groupby("A")["B"].size().sort_index(),
        ...
        self.assert_eq(
            kdf.groupby(("X", "A")).size().sort_index(),
    ...
    def test_diff(self):
        ...
        self.assert_eq(
            kdf.groupby("b").diff().sort_index(), pdf.groupby("b").diff().sort_index()
        )
        self.assert_eq(
            kdf.groupby(["a", "b"]).diff().sort_index(),
        ...
        self.assert_eq(
            kdf.groupby(("x", "b")).diff().sort_index(),
    ...
    def test_rank(self):
        ...
        self.assert_eq(
            kdf.groupby("b").rank().sort_index(), pdf.groupby("b").rank().sort_index()
        )
        self.assert_eq(
            kdf.groupby(["a", "b"]).rank().sort_index(),
        ...
        self.assert_eq(
            kdf.groupby(("x", "b")).rank().sort_index(),
    ...
    def test_cummin(self):
        ...
        self.assert_eq(
            kdf.groupby("b").cummin().sort_index(),
    ...
    def test_cummax(self):
        ...
        self.assert_eq(
            kdf.groupby("b").cummax().sort_index(),
    ...
    def test_cumsum(self):
        ...
        self.assert_eq(
            kdf.groupby("b").cumsum().sort_index(),
    ...
    def test_nsmallest(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "nsmallest do not support multi-index now"
        ):
    ...
    def test_nlargest(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "nlargest do not support multi-index now"
        ):
    ...
    def test_fillna(self):
        ...
        self.assert_eq(
            kdf.groupby("A").fillna(0).sort_index(),
    ...
    def test_ffill(self):
        ...
        self.assert_eq(
            repr(kdf.groupby("A")["B"].ffill().sort_index()),
    ...
    def test_bfill(self):
        ...
        self.assert_eq(
            repr(kdf.groupby("A")["B"].bfill().sort_index()),
    ...
    def test_shift(self):
        ...
        self.assert_eq(
            kdf.groupby("a").shift().sort_index(), pdf.groupby("a").shift().sort_index()
        )
    ...
    def test_apply_with_new_dataframe(self):
        ...
        self.assert_eq(
            kdf.groupby("car_id")
            .apply(lambda _: pd.DataFrame({"column": [0.0]}))
            .sort_index(),
            pdf.groupby("car_id")
            .apply(lambda _: pd.DataFrame({"column": [0.0]}))
            .sort_index(),
    ...
    def test_idxmax(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "idxmax only support one-level index now"
        ):
    ...
    def test_idxmin(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "idxmin only support one-level index now"
        ):
    ...
    def test_missing(self):
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeSeriesGroupBy, inspect.isfunction
        )
        unsupported_functions = [
            name
            for (name, type_) in missing_functions
    ...
    def test_is_multi_agg_with_relabel():
        ...
        assert (
            _is_multi_agg_with_relabel(a_min=("a", "max"), a_max=("a", "min")) is True
        )

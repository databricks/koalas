...
class GroupByTest(ReusedSQLTestCase, TestUtils):
    def test_groupby(self):
        ...
        for as_index in [True, False]:
            self.assert_eq(
                kdf.groupby("a", as_index=as_index).sum(), pdf.groupby("a", as_index=as_index).sum()
            )
            self.assert_eq(
                kdf.groupby("a", as_index=as_index).b.sum(),
            ...
            self.assert_eq(
                kdf.groupby("a", as_index=as_index)[["b", "c"]].sum(),
                pdf.groupby("a", as_index=as_index)[["b", "c"]].sum(),
            )
            self.assert_eq(
                kdf.groupby("a", as_index=as_index)[[]].sum(),
                pdf.groupby("a", as_index=as_index)[[]].sum(),
            )
            self.assert_eq(
                kdf.groupby("a", as_index=as_index)["c"].sum(),
    ...
    def test_split_apply_combine_on_series(self):
        ...
        for ddkey, pdkey in [("b", "b"), (kdf.b, pdf.b), (kdf.b + 1, pdf.b + 1)]:
            for almost, func in funcs:
                self.assert_eq(
                    getattr(kdf.groupby(ddkey).a, func)(),
                    getattr(pdf.groupby(pdkey).a, func)(),
                    almost=almost,
                )
                self.assert_eq(
                    getattr(kdf.groupby(ddkey), func)(),
                    getattr(pdf.groupby(pdkey), func)(),
                    almost=almost,
                )
        ...
        for ddkey, pdkey in [(kdf.b, pdf.b), (kdf.b + 1, pdf.b + 1)]:
            for almost, func in funcs:
                self.assert_eq(
                    getattr(kdf.a.groupby(ddkey), func)(),
                    getattr(pdf.a.groupby(pdkey), func)(),
                    almost=almost,
                )
        ...
        for i in [0, 4, 7]:
            for almost, func in funcs:
                self.assert_eq(
                    getattr(kdf.groupby(kdf.b > i).a, func)(),
                    getattr(pdf.groupby(pdf.b > i).a, func)(),
                    almost=almost,
                )
                self.assert_eq(
                    getattr(kdf.groupby(kdf.b > i), func)(),
                    getattr(pdf.groupby(pdf.b > i), func)(),
                    almost=almost,
                )
    ...
    def test_aggregate(self):
        ...
        for as_index in [True, False]:
            self.assert_eq(
                kdf.groupby("A", as_index=as_index).agg({"B": "min", "C": "sum"}),
                pdf.groupby("A", as_index=as_index).agg({"B": "min", "C": "sum"}),
            )

            self.assert_eq(
                kdf.groupby("A", as_index=as_index).agg({"B": ["min", "max"], "C": "sum"}),
                pdf.groupby("A", as_index=as_index).agg({"B": ["min", "max"], "C": "sum"}),
            )

        expected_error_message = (
            r"aggs must be a dict mapping from column name \(string\) "
    ...
    def test_all_any(self):
        ...
        for as_index in [True, False]:
            self.assert_eq(
                kdf.groupby("A", as_index=as_index).all(), pdf.groupby("A", as_index=as_index).all()
            )
            self.assert_eq(
                kdf.groupby("A", as_index=as_index).any(), pdf.groupby("A", as_index=as_index).any()
            )

            self.assert_eq(
                kdf.groupby("A", as_index=as_index).all().B,
                pdf.groupby("A", as_index=as_index).all().B,
            )
            self.assert_eq(
                kdf.groupby("A", as_index=as_index).any().B,
    ...
    def test_nunique(self):
        ...
        self.assert_eq(
            kdf.groupby("a").agg({"b": "nunique"}), pdf.groupby("a").agg({"b": "nunique"})
        )
        self.assert_eq(kdf.groupby("a").nunique(), pdf.groupby("a").nunique())
        self.assert_eq(
            kdf.groupby("a").nunique(dropna=False), pdf.groupby("a").nunique(dropna=False)
        )
        self.assert_eq(kdf.groupby("a")["b"].nunique(), pdf.groupby("a")["b"].nunique())
        self.assert_eq(
            kdf.groupby("a")["b"].nunique(dropna=False), pdf.groupby("a")["b"].nunique(dropna=False)
        )

        for as_index in [True, False]:
            self.assert_eq(
                kdf.groupby("a", as_index=as_index).agg({"b": "nunique"}),
    ...
    def test_fillna(self):
        ...
        kdf = koalas.DataFrame(pdf)
        self.assert_eq(kdf.groupby("A").fillna(0), pdf.groupby("A").fillna(0))
        self.assert_eq(
            kdf.groupby("A").fillna(method="bfill"), pdf.groupby("A").fillna(method="bfill")
        )
        self.assert_eq(
            kdf.groupby("A").fillna(method="ffill"), pdf.groupby("A").fillna(method="ffill")
        )
    ...
    def test_ffill(self):
        ...
        if LooseVersion(pd.__version__) <= LooseVersion("0.24.2"):
            self.assert_eq(kdf.groupby("A").ffill(), pdf.groupby("A").ffill().drop("A", 1))
        else:
            self.assert_eq(kdf.groupby("A").ffill(), pdf.groupby("A").ffill())
        self.assert_eq(repr(kdf.groupby("A")["B"].ffill()), repr(pdf.groupby("A")["B"].ffill()))
    ...
    def test_bfill(self):
        ...
        kdf = koalas.DataFrame(pdf)
        if LooseVersion(pd.__version__) <= LooseVersion("0.24.2"):
            self.assert_eq(kdf.groupby("A").bfill(), pdf.groupby("A").bfill().drop("A", 1))
        else:
            self.assert_eq(kdf.groupby("A").bfill(), pdf.groupby("A").bfill())
        self.assert_eq(repr(kdf.groupby("A")["B"].bfill()), repr(pdf.groupby("A")["B"].bfill()))
    ...
    def test_idxmax(self):
        pdf = pd.DataFrame(
            {"a": [1, 1, 2, 2, 3], "b": [1, 2, 3, 4, 5], "c": [5, 4, 3, 2, 1]},
            columns=["a", "b", "c"],
        )
        kdf = koalas.DataFrame(pdf)
        self.assert_eq(pdf.groupby(["a"]).idxmax(), kdf.groupby(["a"]).idxmax().sort_index())
        self.assert_eq(
            pdf.groupby(["a"]).idxmax(skipna=False),
    ...
    def test_idxmin(self):
        ...
        kdf = koalas.DataFrame(pdf)
        self.assert_eq(pdf.groupby(["a"]).idxmin(), kdf.groupby(["a"]).idxmin().sort_index())
        self.assert_eq(
            pdf.groupby(["a"]).idxmin(skipna=False),

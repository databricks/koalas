...
class SeriesTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def _test_value_counts(self):
        ...
        self.assert_eq(
            kser.value_counts(normalize=True),
            pser.value_counts(normalize=True),
            almost=True,
        )
        self.assert_eq(
            kser.value_counts(ascending=True),
            pser.value_counts(ascending=True),
            almost=True,
        )
        self.assert_eq(
        ...
        self.assert_eq(
            kdf.a.value_counts(normalize=True),
            pdf.a.value_counts(normalize=True),
            almost=True,
        )
        self.assert_eq(
            kdf.a.value_counts(ascending=True),
            pdf.a.value_counts(ascending=True),
            almost=True,
        )
        self.assert_eq(
    ...
    def test_reset_index_with_default_index_types(self):
        ...
        with ks.option_context("compute.default_index_type", "distributed"):
            ...
            self.assert_eq(
                kser.reset_index().to_pandas().reset_index(drop=True),
                pser.reset_index(),
            )
    ...
    def test_sort_values(self):
        ...
        self.assert_eq(
            repr(kser.sort_values(ascending=False)),
            repr(pser.sort_values(ascending=False)),
        )
        self.assert_eq(
            repr(kser.sort_values(na_position="first")),
    ...
    def test_sort_index(self):
        ...
        self.assertRaises(
            NotImplementedError, lambda: kser.sort_index(kind="mergesort")
        )
        ...
        self.assert_eq(
            kser.sort_index(na_position="first"),
        ...
        self.assert_eq(kser.sort_index(), pser.sort_index(), almost=True)
        self.assert_eq(
            kser.sort_index(level=[1, 0]), pser.sort_index(level=[1, 0]), almost=True
        )
    ...
    def test_missing(self):
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeSeries, inspect.isfunction
        )
    ...
    def test_append(self):
        ...
        self.assert_eq(
            kser1.append(kser2, ignore_index=True),
    ...
    def test_cummin(self):
        ...
        self.assertEqual(
            repr(pser.cummin(skipna=False)), repr(kser.cummin(skipna=False))
        )
    ...
    def test_cummax(self):
        ...
        self.assertEqual(
            repr(pser.cummax(skipna=False)), repr(kser.cummax(skipna=False))
        )
    ...
    def test_cumsum(self):
        ...
        self.assertEqual(
            repr(pser.cumsum(skipna=False)), repr(kser.cumsum(skipna=False))
        )
    ...
    def test_cumprod(self):
        ...
        self.assertEqual(
            repr(pser.cumprod(skipna=False)), repr(kser.cumprod(skipna=False))
        )
    ...
    def test_rank(self):
        ...
        self.assertEqual(repr(pser.rank()), repr(kser.rank().sort_index()))
        self.assertEqual(
            repr(pser.rank(ascending=False)),
            repr(kser.rank(ascending=False).sort_index()),
        )
        self.assertEqual(
            repr(pser.rank(method="min")), repr(kser.rank(method="min").sort_index())
        )
        self.assertEqual(
            repr(pser.rank(method="max")), repr(kser.rank(method="max").sort_index())
        )
        self.assertEqual(
            repr(pser.rank(method="first")),
            repr(kser.rank(method="first").sort_index()),
        )
        self.assertEqual(
            repr(pser.rank(method="dense")),
    ...
    def test_quantile(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "q must be a float of an array of floats;"
        ):
    ...
    def test_idxmax(self):
        ...
        self.assertEqual(kser.idxmax(), pser.idxmax())
        self.assertEqual(
            repr(kser.idxmax(skipna=False)), repr(pser.idxmax(skipna=False))
        )
    ...
    def test_idxmin(self):
        ...
        self.assertEqual(kser.idxmin(), pser.idxmin())
        self.assertEqual(
            repr(kser.idxmin(skipna=False)), repr(pser.idxmin(skipna=False))
        )
    ...
    def test_shift(self):
        ...
        if LooseVersion(pd.__version__) < LooseVersion("0.24.2"):
            ...
            self.assertEqual(
                repr(kser.shift(periods=2, fill_value=0)),
    ...
    def test_duplicates(self):
        ...
        self.assert_eq(
            pser.drop_duplicates().sort_values(), kser.drop_duplicates().sort_values()
        )
    ...
    def test_where(self):
        ...
        self.assert_eq(
            repr(pser1.where(pser1 > 3)), repr(kser1.where(kser1 > 3).sort_index())
        )
    ...
    def test_mask(self):
        ...
        self.assert_eq(
            repr(pser1.mask(pser1 > 3)), repr(kser1.mask(kser1 > 3).sort_index())
        )
    ...
    def test_truncate(self):
        ...
        self.assert_eq(
            kser1.truncate(2, 5, copy=False), pser1.truncate(2, 5, copy=False)
        )
        self.assert_eq(kser2.truncate(4, 6), pser2.truncate(4, 6))
        self.assert_eq(
            kser2.truncate(4, 6, copy=False), pser2.truncate(4, 6, copy=False)
        )
    ...
    def test_getitem(self):
        ...
        pser = pd.Series(
            [45, 200, 1.2, 30, 250, 1.5, 320, 1, 0.3], name="0", index=midx
        )
        kser = ks.Series(pser)
    ...
    def test_pct_change(self):
        ...
        self.assert_eq(
            kser.pct_change(periods=-1), pser.pct_change(periods=-1), almost=True
        )
        self.assert_eq(
            kser.pct_change(periods=-100000000),
            pser.pct_change(periods=-100000000),
            almost=True,
        )
        self.assert_eq(
            kser.pct_change(periods=100000000),
        ...
        self.assert_eq(
            kser.pct_change(periods=2), pser.pct_change(periods=2), almost=True
        )
        self.assert_eq(
            kser.pct_change(periods=-1), pser.pct_change(periods=-1), almost=True
        )

...
class RollingTest(ReusedSQLTestCase, TestUtils):
    ...
    def _test_rolling_func(self, f):
        ...
        pser = kser.to_pandas()
        self.assert_eq(
            repr(getattr(kser.rolling(2), f)()), repr(getattr(pser.rolling(2), f)())
        )
        ...
        pdf = kdf.to_pandas()
        self.assert_eq(
            repr(getattr(kdf.rolling(2), f)()), repr(getattr(pdf.rolling(2), f)())
        )
    ...
    def _test_groupby_rolling_func(self, f):
        ...
        self.assert_eq(
            repr(
                getattr(
                    kdf.groupby([("a", "x"), ("a", "y")]).rolling(2), f
                )().sort_index()
            ),
            repr(
                getattr(
                    pdf.groupby([("a", "x"), ("a", "y")]).rolling(2), f
                )().sort_index()
            ),
        )

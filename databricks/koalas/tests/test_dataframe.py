...
class DataFrameTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def test_dataframe(self):
        ...
        expected = pd.Series(
            [2, 3, 4, 5, 6, 7, 8, 9, 10], index=[0, 1, 3, 5, 6, 8, 9, 9, 9], name="(a + 1)"
        )  # TODO: name='a'
        ...
        df = pd.DataFrame({"a": [1, 2, 3, 4, 5, 6, 7, 8, 9], "b": [4, 5, 6, 3, 2, 1, 0, 0, 0]})
    ...
    def test_fillna(self):
        ...
        pdf = pd.DataFrame(
            {"x": [1, 2, 3, 4, 5, 6], "y": [6, 5, 4, 3, 2, 1], "z": [1, 2, 3, 4, np.nan, np.nan]}
        ).set_index(["x", "y"])
        kdf = ks.from_pandas(pdf)
        # check multi index
        self.assert_eq(pdf.fillna(method="bfill"), kdf.fillna(method="bfill"))
    ...
    def test_isnull(self):
        pdf = pd.DataFrame(
            {"x": [1, 2, 3, 4, None, 6], "y": list("abdabd")}, index=[10, 20, 30, 40, 50, 60]
        )
        kdf = ks.from_pandas(pdf)
    ...
    def test_sort_values(self):
        pdf = pd.DataFrame({"a": [1, 2, 3, 4, 5, None, 7], "b": [7, 6, 5, 4, 3, 2, 1]})
        kdf = ks.from_pandas(pdf)
    ...
    def test_merge_raises(self):
        ...
        with self.assertRaisesRegex(AnalysisException, 'Cannot resolve column name "`id`"'):
    ...
    def test_replace(self):
        ...
        self.assert_eq(repr(kdf.replace([0, 1, 2, 3], 4)), repr(pdf.replace([0, 1, 2, 3], 4)))

        self.assert_eq(
            repr(kdf.replace([0, 1, 2, 3], [4, 3, 2, 1])),
            repr(pdf.replace([0, 1, 2, 3], [4, 3, 2, 1])),
        )

        self.assert_eq(repr(kdf.replace({0: 10, 1: 100})), repr(pdf.replace({0: 10, 1: 100})))

        self.assert_eq(
            repr(kdf.replace({"A": 0, "B": 5}, 100)), repr(pdf.replace({"A": 0, "B": 5}, 100))
        )

        self.assert_eq(
            repr(kdf.replace({"A": {0: 100, 4: 400}})), repr(pdf.replace({"A": {0: 100, 4: 400}}))
        )
    ...
    def test_pivot_table(self):
        ...
        np.testing.assert_equal(
            kdf.pivot_table(columns="a", values="b").to_numpy(),
    ...
    def test_rank(self):
        pdf = pd.DataFrame(
            data={"col1": [1, 2, 3, 1], "col2": [3, 4, 3, 1]}, columns=["col1", "col2"]
        )
        kdf = ks.from_pandas(pdf)
        self.assert_eq(pdf.rank(), kdf.rank().sort_index())
        self.assert_eq(pdf.rank(), kdf.rank().sort_index())
        self.assert_eq(pdf.rank(ascending=False), kdf.rank(ascending=False).sort_index())
        ...
        self.assert_eq(pdf.rank(method="dense"), kdf.rank(method="dense").sort_index())
    ...
    def test_shift(self):
        ...
        self.assert_eq(pdf, kdf.shift(periods=3, fill_value=0).sort_index())
    ...
    def test_diff(self):
        ...
        kdf = ks.from_pandas(pdf)
        self.assert_eq(pdf.diff(), kdf.diff().sort_index())
    ...
    def test_duplicated(self):
        ...
        kdf = ks.from_pandas(pdf)
        self.assertEqual(
            repr(pd.Series(pdf.duplicated(), name="0")), repr(kdf.duplicated().sort_index())
        )
        self.assertEqual(
            repr(pd.Series(pdf.duplicated(), name="0")), repr(kdf.duplicated().sort_index())
        )
        self.assertEqual(
            repr(pd.Series(pdf.duplicated(keep="last"), name="0")),
            repr(kdf.duplicated(keep="last").sort_index()),
        )
        self.assertEqual(
            repr(pd.Series(pdf.duplicated(keep=False), name="0")),
            repr(kdf.duplicated(keep=False).sort_index()),
        )
        self.assertEqual(
            repr(pd.Series(pdf.duplicated(subset=["a"]), name="0")),

...
class DataFrameTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def test_dataframe_column_level_name(self):
        ...
        pdf = pd.DataFrame(
            [[1, 2, 3], [4, 5, 6]], columns=column, index=np.random.rand(2)
        )
        kdf = ks.from_pandas(pdf)
    ...
    def test_dataframe_multiindex_names_level(self):
        ...
        self.assert_eq(
            kdf["X"]["A"].to_pandas().columns.names, pdf["X"]["A"].columns.names
        )
        self.assert_eq(kdf[("X", "A")], pdf[("X", "A")])
        self.assert_eq(kdf[("X", "A")].columns.names, pdf[("X", "A")].columns.names)
        self.assert_eq(
    ...
    def test_reset_index_with_default_index_types(self):
        ...
        with ks.option_context("compute.default_index_type", "distributed"):
            ...
            self.assert_eq(
                kdf.reset_index().to_pandas().reset_index(drop=True), pdf.reset_index()
            )
    ...
    def test_reset_index_with_multiindex_columns(self):
        ...
        self.assert_eq(
            kdf.reset_index(level="class", col_level=1),
    ...
    def test_empty_dataframe(self):
        pdf = pd.DataFrame(
            {"a": pd.Series([], dtype="i1"), "b": pd.Series([], dtype="str")}
        )
    ...
    def test_rename_columns(self):
        ...
        msg = (
            "Length mismatch: Expected axis has 2 elements, new values have 4 elements"
        )
    ...
    def test_rename_dataframe(self):
        ...
        kdf3 = ks.DataFrame(
            [[1, 2], [3, 4], [5, 6], [7, 8]], index=idx, columns=list("ab")
        )
    ...
    def test_drop(self):
        pdf = pd.DataFrame(
            {"x": [1, 2], "y": [3, 4], "z": [5, 6]}, index=np.random.rand(2)
        )
        kdf = ks.from_pandas(pdf)
        ...
        expected_output = pd.DataFrame(
            {"y": [3, 4], "z": [5, 6]}, index=kdf.index.to_pandas()
        )
        ...
        self.assert_eq(
            kdf.drop(columns=[("a", "x"), "b"]), pdf.drop(columns=[("a", "x"), "b"])
        )
    ...
    def test_fillna(self):
        ...
        self.assert_eq(
            kdf.fillna({"x": -1, "y": -2, "z": -5}),
            pdf.fillna({"x": -1, "y": -2, "z": -5}),
        )
        self.assert_eq(pdf.fillna(method="ffill"), kdf.fillna(method="ffill"))
        self.assert_eq(
            pdf.fillna(method="ffill", limit=2), kdf.fillna(method="ffill", limit=2)
        )
        self.assert_eq(pdf.fillna(method="bfill"), kdf.fillna(method="bfill"))
        self.assert_eq(
            pdf.fillna(method="bfill", limit=2), kdf.fillna(method="bfill", limit=2)
        )
        ...
        with self.assertRaisesRegex(
            ValueError, "limit parameter for value is not support now"
        ):
        ...
        if sys.version_info >= (3, 6):
            # flaky in Python 3.5.
            self.assert_eq(
                kdf.fillna({"x": -1, ("x", "b"): -2}),
                pdf.fillna({"x": -1, ("x", "b"): -2}),
            )
            self.assert_eq(
                kdf.fillna({("x", "b"): -2, "x": -1}),
    ...
    def test_nunique(self):
        pdf = pd.DataFrame(
            {"A": [1, 2, 3], "B": [np.nan, 3, np.nan]}, index=np.random.rand(3)
        )
        kdf = ks.from_pandas(pdf)
    ...
    def test_sort_values(self):
        ...
        self.assert_eq(
            repr(kdf.sort_values(["b", "a"])), repr(pdf.sort_values(["b", "a"]))
        )
        self.assert_eq(
        ...
        self.assertRaises(
            ValueError, lambda: kdf.sort_values(["b", "a"], ascending=[False])
        )
        ...
        self.assertRaises(
            ValueError, lambda: kdf.sort_values(["b", "a"], na_position="invalid")
        )

        self.assert_eq(
            kdf.sort_values("b", inplace=True), pdf.sort_values("b", inplace=True)
        )
    ...
    def test_sort_index(self):
        ...
        self.assert_eq(
            kdf.sort_index(na_position="first"), pdf.sort_index(na_position="first")
        )
    ...
    def test_nlargest(self):
        ...
        self.assert_eq(
            kdf.nlargest(n=5, columns=["a", "b"]), pdf.nlargest(5, columns=["a", "b"])
        )
    ...
    def test_nsmallest(self):
        ...
        self.assert_eq(
            kdf.nsmallest(n=5, columns=["a", "b"]), pdf.nsmallest(5, columns=["a", "b"])
        )
    ...
    def test_xs(self):
        ...
        self.assert_eq(
            kdf.xs(("mammal", "dog", "walks")), pdf.xs(("mammal", "dog", "walks"))
        )
    ...
    def test_missing(self):
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeDataFrame, inspect.isfunction
        )
        unsupported_functions = [
            name
            for (name, type_) in missing_functions
    ...
    def test_merge(self):
        ...
        for how in ["inner", "left", "right", "outer"]:
            ...
            check(
                lambda left, right: left.merge(
                    right, left_on="lkey", right_on="rkey", how=how
                )
            )
        ...
        if LooseVersion(pd.__version__) >= LooseVersion("0.24.0"):
            ...
            check(
                lambda left, right: left.set_index("x").merge(
                    right, left_index=True, right_on="x"
                ),
                right_kser,
                right_ps,
            )
            ...
            for how in ["inner", "left", "right", "outer"]:
                check(
                    lambda left, right: left.merge(right, how=how), right_kser, right_ps
                )
                check(
                    lambda left, right: left.merge(
                        right, left_on="x", right_on="x", how=how
                    ),
                    right_kser,
                    right_ps,
                )
    ...
    def test_merge_raises(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Must pass right_on or right_index=True"
        ):
        ...
        with self.assertRaisesRegex(
            ValueError, "['inner', 'left', 'right', 'full', 'outer']"
        ):
    ...
    def test_append(self):
        ...
        self.assert_eq(
            kdf.append(kdf, ignore_index=True), pdf.append(pdf, ignore_index=True)
        )
        ...
        multi_index_pdf = pd.DataFrame(
            [[1, 2], [3, 4]], columns=list("AB"), index=[[2, 3], [4, 5]]
        )
        ...
        self.assert_eq(
            multi_index_kdf.append(multi_index_kdf),
        ...
        self.assert_eq(
            ...
            multi_index_pdf.append(
                multi_index_pdf, ignore_index=True, verify_integrity=True
            ),
        )
    ...
    def test_add_prefix(self):
        pdf = pd.DataFrame(
            {"A": [1, 2, 3, 4], "B": [3, 4, 5, 6]}, index=np.random.rand(4)
        )
        kdf = ks.from_pandas(pdf)
    ...
    def test_replace(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Length of to_replace and value must be same"
        ):
            kdf.replace(to_replace=["Ironman"], value=["Spiderman", "Doctor Strange"])

        self.assert_eq(
            kdf.replace("Ironman", "Spiderman"), pdf.replace("Ironman", "Spiderman")
        )
        self.assert_eq(
        ...
        self.assert_eq(
            kdf.replace([0, 1, 2, 3, 5, 6], 4), pdf.replace([0, 1, 2, 3, 5, 6], 4)
        )
        ...
        self.assert_eq(
            kdf.replace({0: 10, 1: 100, 7: 200}), pdf.replace({0: 10, 1: 100, 7: 200})
        )

        self.assert_eq(
            kdf.replace({"A": 0, "B": 5}, 100), pdf.replace({"A": 0, "B": 5}, 100)
        )

        self.assert_eq(
            kdf.replace({"A": {0: 100, 4: 400}}), pdf.replace({"A": {0: 100, 4: 400}})
        )
    ...
    def test_update(self):
        ...
        def get_data(left_columns=None, right_columns=None):
            left_pdf = pd.DataFrame(
                {"A": ["1", "2", "3", "4"], "B": ["100", "200", np.nan, np.nan]},
                columns=["A", "B"],
            )
            right_pdf = pd.DataFrame(
            ...
            left_kdf = ks.DataFrame(
                {"A": ["1", "2", "3", "4"], "B": ["100", "200", None, None]},
                columns=["A", "B"],
            )
            right_kdf = ks.DataFrame(
                {"B": ["x", None, "y", None], "C": ["100", "200", "300", "400"]},
                columns=["B", "C"],
            )
        ...
        self.assert_eq(
            left_pdf.sort_values(by=["A", "B"]), left_kdf.sort_values(by=["A", "B"])
        )
    ...
    def test_pivot_table_errors(self):
        ...
        with self.assertRaisesRegex(NotImplementedError, msg):
            kdf.pivot_table(
                columns="a", values=["b", "e"], aggfunc={"b": "mean", "e": "sum"}
            )

        msg = "columns should be string."
        with self.assertRaisesRegex(ValueError, msg):
            kdf.pivot_table(
                columns=["a"], values=["b"], aggfunc={"b": "mean", "e": "sum"}
            )
    ...
    def test_transpose(self):
        ...
        pdf1 = pd.DataFrame(
            data={"col1": [1, 2], "col2": [3, 4]}, columns=["col1", "col2"]
        )
        ...
        self.assert_eq(
            repr(pdf2.transpose().sort_index()), repr(kdf2.transpose().sort_index())
        )
    ...
    def _test_cumprod(self, pdf, kdf):
        ...
        self.assertEqual(
            repr(pdf.cumprod(skipna=False)), repr(kdf.cumprod(skipna=False))
        )
    ...
    def test_drop_duplicates(self):
        ...
        # inplace is False
        self.assert_eq(
            pdf.drop_duplicates().sort_index(), kdf.drop_duplicates().sort_index()
        )
        self.assert_eq(
            pdf.drop_duplicates("a").sort_index(), kdf.drop_duplicates("a").sort_index()
        )
        self.assert_eq(
    ...
    def test_reindex(self):
        ...
        self.assertRaises(
            TypeError, lambda: kdf.reindex(columns=["numbers", "2", "3"], axis=1)
        )
        self.assertRaises(
            TypeError, lambda: kdf.reindex(columns=["numbers", "2", "3"], axis=2)
        )
    ...
    def test_melt(self):
        ...
        self.assert_eq(
            ...
            .sort_values(["variable", "value"])
            .reset_index(drop=True),
            pdf.melt(id_vars=["A"], value_vars=["C"]).sort_values(
                ["variable", "value"]
            ),
        )
        self.assert_eq(
        ...
        self.assert_eq(
            ...
            .sort_values(["variable_0", "variable_1", "value"])
            .reset_index(drop=True),
            pdf.melt(id_vars=[("X", "A")]).sort_values(
    ...
    def test_rank(self):
        ...
        self.assert_eq(
            pdf.rank(ascending=False).sort_index(),
            kdf.rank(ascending=False).sort_index(),
        )
        self.assert_eq(
            pdf.rank(method="min").sort_index(), kdf.rank(method="min").sort_index()
        )
        self.assert_eq(
            pdf.rank(method="max").sort_index(), kdf.rank(method="max").sort_index()
        )
        self.assert_eq(
            pdf.rank(method="first").sort_index(), kdf.rank(method="first").sort_index()
        )
        self.assert_eq(
            pdf.rank(method="dense").sort_index(), kdf.rank(method="dense").sort_index()
        )
    ...
    def test_round(self):
        ...
        self.assert_eq(pdf.round(2), kdf.round(2))
        self.assert_eq(
            pdf.round({("X", "A"): 1, ("Y", "C"): 2}),
            kdf.round({("X", "A"): 1, ("Y", "C"): 2}),
        )
        self.assert_eq(
            pdf.round({("X", "A"): 1, "Y": 2}), kdf.round({("X", "A"): 1, "Y": 2})
        )
        self.assert_eq(pdf.round(pser), kdf.round(kser))
    ...
    def test_duplicated(self):
        pdf = pd.DataFrame(
            {"a": [1, 1, 1, 3], "b": [1, 1, 1, 4], "c": [1, 1, 1, 5]},
            index=np.random.rand(4),
        )
        kdf = ks.from_pandas(pdf)

        self.assert_eq(
            pd.Series(pdf.duplicated(), name="0").sort_index(),
        ...
        with self.assertRaisesRegex(
            ValueError, "'keep' only support 'first', 'last' and False"
        ):
        ...
        kdf.index.name = "x"
        self.assert_eq(
            pd.Series(pdf.duplicated(), name="x").sort_index(),
            kdf.duplicated().sort_index(),
        )
    ...
    def test_filter(self):
        ...
        self.assert_eq(
            kdf.filter(like="b", axis="index"), pdf.filter(like="b", axis="index")
        )
        self.assert_eq(
            kdf.filter(like="c", axis="columns"), pdf.filter(like="c", axis="columns")
        )
        ...
        self.assert_eq(
            kdf.filter(regex="b.*", axis="columns"),
    ...
    def test_apply(self):
        ...
        self.assert_eq(
            kdf.apply(lambda x: x + 1).sort_index(),
        ...
        with self.assertRaisesRegex(
            TypeError, "The given function.*1 or 'column'; however"
        ):
        ...
        with self.assertRaisesRegex(
            TypeError, "The given function.*0 or 'index'; however"
        ):
    ...
    def test_empty_timestamp(self):
        ...
        self.assert_eq(
            kdf[kdf["t"] != kdf["t"]].dtypes, pdf[pdf["t"] != pdf["t"]].dtypes
        )
    ...
    def test_where(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "type of cond must be a DataFrame or Series"
        ):
            kdf.where(1)
    ...
    def test_query(self):
        ...
        kdf.columns = columns
        with self.assertRaisesRegex(
            ValueError, "Doesn't support for MultiIndex columns"
        ):
            kdf.query("('A', 'Z') > ('B', 'X')")

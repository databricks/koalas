...
class DataFrameConversionTest(ReusedSQLTestCase, SQLTestUtils, TestUtils):
    ...
    def test_to_excel(self):
        with self.temp_dir() as dirpath:
            ...
            pdf = pd.DataFrame(
                {"a": [1, None, 3], "b": ["one", "two", None],}, index=[0, 1, 3]
            )
    ...
    def test_to_clipboard(self):
        ...
        self.assert_eq(
            kdf.to_clipboard(sep=";", index=False),
    ...
    def test_to_latex(self):
        ...
        self.assert_eq(
            kdf.to_latex(float_format="%.1f"), pdf.to_latex(float_format="%.1f")
        )
    ...
    def test_to_records(self):
        if LooseVersion(pd.__version__) >= LooseVersion("0.24.0"):
            ...
            self.assert_array_eq(
                kdf.to_records(index=False), pdf.to_records(index=False)
            )
            self.assert_array_eq(
    ...
    def test_from_records(self):
        ...
        self.assert_eq(
            repr(ks.DataFrame.from_records(np.eye(3))),

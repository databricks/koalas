...
class OpsOnDiffFramesEnabledTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def test_arithmetic(self):
        ...
        self.assert_eq(
            ...
            (pdf1.a - pdf2.b).rename("a").sort_index(),
            almost=True,
        )
        ...
        # DataFrame
        self.assert_eq(
            (kdf1 + kdf2).sort_index(), (pdf1 + pdf2).sort_index(), almost=True
        )
        ...
        # MultiIndex Series
        self.assert_eq(
            (kser1 + kser2).sort_index(), (pser1 + pser2).sort_index(), almost=True
        )
    ...
    def test_loc_getitem_boolean_series(self):
        ...
        self.assert_eq(
            pdf1.loc[pdf2.A > -3].sort_index(), kdf1.loc[kdf2.A > -3].sort_index()
        )

        self.assert_eq(
            pdf1.A.loc[pdf2.A > -3].sort_index(), kdf1.A.loc[kdf2.A > -3].sort_index()
        )
    ...
    def test_different_columns(self):
        ...
        self.assert_eq(
            (kdf1 + kdf4).sort_index(), (pdf1 + pdf4).sort_index(), almost=True
        )
    ...
    def test_multi_index_arithmetic(self):
        ...
        # DataFrame
        self.assert_eq(
            (kdf5 + kdf6).sort_index(), (pdf5 + pdf6).sort_index(), almost=True
        )
    ...
    def test_where(self):
        ...
        self.assert_eq(
            repr(pdf1.where(pdf2 > 100)), repr(kdf1.where(kdf2 > 100).sort_index())
        )
    ...
    def test_mask(self):
        ...
        self.assert_eq(
            repr(pdf1.mask(pdf2 < 100)), repr(kdf1.mask(kdf2 < 100).sort_index())
        )
        ...
        self.assert_eq(
            repr(pdf1.mask(pdf2 > -250)), repr(kdf1.mask(kdf2 > -250).sort_index())
        )

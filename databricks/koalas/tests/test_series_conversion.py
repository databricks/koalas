...
class SeriesConversionTest(ReusedSQLTestCase, SQLTestUtils):
    ...
    def test_to_clipboard(self):
        ...
        self.assert_eq(
            kser.to_clipboard(sep=",", index=False),
    ...
    def test_to_latex(self):
        ...
        self.assert_eq(
            kser.to_latex(float_format="%.1f"), pser.to_latex(float_format="%.1f")
        )
        self.assert_eq(kser.to_latex(sparsify=False), pser.to_latex(sparsify=False))
        self.assert_eq(
            kser.to_latex(index_names=False), pser.to_latex(index_names=False)
        )

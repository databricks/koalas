...
class ReshapeTest(ReusedSQLTestCase):
    def test_get_dummies(self):
        for pdf_or_ps in [
            ...
            self.assert_eq(
                ks.get_dummies(kdf_or_kser), pd.get_dummies(pdf_or_ps), almost=True
            )
    ...
    def test_get_dummies_object(self):
        ...
        self.assert_eq(
            ks.get_dummies(kdf, columns=["b"]),
            pd.get_dummies(pdf, columns=["b"]),
            almost=True,
        )
    ...
    def test_get_dummies_kwargs(self):
        ...
        self.assert_eq(
            ...
            pd.get_dummies(pser, dummy_na=True),
            almost=True,
        )

    def test_get_dummies_prefix(self):
        pdf = pd.DataFrame(
            {"A": ["a", "b", "a"], "B": ["b", "a", "c"], "D": [0, 0, 1],}
        )
        kdf = ks.from_pandas(pdf)
        ...
        with self.assertRaisesRegex(
            ValueError, "Length of 'prefix' \\(1\\) .* \\(2\\)"
        ):
            ks.get_dummies(kdf, prefix=["foo"])
        with self.assertRaisesRegex(
            ValueError, "Length of 'prefix' \\(2\\) .* \\(1\\)"
        ):
        ...
        self.assert_eq(
            ks.get_dummies(kser, prefix="foo"),
            pd.get_dummies(pser, prefix="foo"),
            almost=True,
        )
    ...
    def test_get_dummies_multiindex_columns(self):
        ...
        self.assertRaises(
            ValueError, lambda: ks.get_dummies(kdf, columns=[("x",), "c"])
        )

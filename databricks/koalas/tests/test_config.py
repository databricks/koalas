...
class ConfigTest(ReusedSQLTestCase):
    def setUp(self):
        config._options_dict["test.config"] = Option(
            key="test.config", doc="", default="default"
        )
    ...
    def test_namespace_access(self):
        try:
            self.assertEqual(
                ks.options.compute.max_rows, ks.get_option("compute.max_rows")
            )
            ...
            self.assertRaisesRegex(
                config.OptionError, "No such option", lambda: ks.options.compu
            )
            self.assertRaisesRegex(

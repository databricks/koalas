...
class IndexesTest(ReusedSQLTestCase, TestUtils):
    ...
    def test_index(self):
        for pdf in [
            ...
            pd.DataFrame(np.random.randn(10, 5), columns=list("abcde")).set_index(
                ["a", "b"]
            ),
        ]:
    ...
    def test_multi_index_getattr(self):
        ...
        expected_error_message = "'MultiIndex' object has no attribute '{}'".format(
            item
        )
        with self.assertRaisesRegex(AttributeError, expected_error_message):
            kidx.__getattr__(item)
    ...
    def test_to_frame(self):
        ...
        self.assert_eq(
            repr(kidx.to_frame(index=False)), repr(pidx.to_frame(index=False))
        )
    ...
    def test_missing(self):
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeIndex, inspect.isfunction
        )
        ...
        missing_functions = inspect.getmembers(
            _MissingPandasLikeMultiIndex, inspect.isfunction
        )
        unsupported_functions = [
            name
            for (name, type_) in missing_functions
    ...
    def test_multi_index_not_supported(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "cannot perform any with this index type"
        ):
    ...
    def test_multiindex_drop(self):
        ...
        self.assert_eq(
            pidx.drop(["x", "y"], level="level2"), kidx.drop(["x", "y"], level="level2")
        )
    ...
    def test_sort_values(self):
        ...
        self.assert_eq(
            pidx.sort_values(ascending=False), kidx.sort_values(ascending=False)
        )
    ...
    def test_index_drop_duplicates(self):
        ...
        kidx = ks.Index([1, 1, 2])
        self.assert_eq(
            pidx.drop_duplicates().sort_values(), kidx.drop_duplicates().sort_values()
        )

        pidx = pd.MultiIndex.from_tuples(
            [(1, 1), (1, 1), (2, 2)], names=["level1", "level2"]
        )
        kidx = ks.MultiIndex.from_tuples(
            [(1, 1), (1, 1), (2, 2)], names=["level1", "level2"]
        )
    ...
    def test_multiindex_isna(self):
        ...
        with self.assertRaisesRegex(
            NotImplementedError, "notna is not defined for MultiIndex"
        ):
            kidx.notna()
    ...
    def test_append(self):
        ...
        expected_error_message = (
            r"append\(\) between Index & MultiIndex currently is not supported"
        )
    ...
    def test_monotonic(self):
        ...
        # more depth tests
        datas.append(
            [("x", "d", "o"), ("y", "c", "p"), ("y", "c", "q"), ("z", "a", "r")]
        )
        datas.append(
            [("x", "d", "o"), ("y", "c", "q"), ("y", "c", "p"), ("z", "a", "r")]
        )
        datas.append(
            [("x", "d", "o"), ("y", "c", None), ("y", "c", "q"), ("z", "a", "r")]
        )
        datas.append(
            [("x", "d", "o"), ("y", "c", "p"), ("y", "c", None), ("z", "a", "r")]
        )
        datas.append(
            [("x", "d", "o"), ("y", "c", None), ("y", "c", None), ("z", "a", "r")]
        )

...
class AtIndexer(IndexerLike):
    ...
    def __getitem__(self, key):
        ...
        if not (col_sel is None or isinstance(col_sel, tuple) or not is_list_like(col_sel)):
            raise ValueError("At based indexing on multi-index can only have tuple values")
        if col_sel is not None:
            col_sel = validate_name_like_tuple(col_sel)
    ...
    def __setitem__(self, key, value):
        ...
        if self._is_series:
            ...
            for label in missing_keys:
                label = validate_name_like_tuple(label)
    ...
    **Getting values on a DataFrame with an index that has integer labels**
    ...
    def _select_cols_by_iterable(
        ...
        if all(isinstance(key, Series) for key in cols_sel):
            ...
            not isinstance(key, tuple) for key in cols_sel
        ):
            raise TypeError(
                "Expected tuple, got {}".format(
                    type(set(key for key in cols_sel if not isinstance(key, tuple)).pop())
                )
            )
        else:
    ...
    def _select_cols_else(
        ...
        cols_sel = validate_name_like_tuple(cols_sel)

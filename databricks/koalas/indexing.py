...
class AtIndexer(_IndexerLike):
    ...
    def __getitem__(self, key):
        ...
        if not (
            ...
            or (
                isinstance(col_sel, tuple)
                and all(isinstance(col, str) for col in col_sel)
            )
        ):
            raise ValueError(
                "At based indexing on multi-index can only have tuple values"
    ...
    def __getitem__(self, key):
        ...
        if self._is_series:
            ...
            if cond is None and limit is None and returns_series:
                return Series(
                    self._internal.copy(
                        scol=column_scols[0], column_labels=[column_labels[0]]
                    ),
        ...
        if returns_series:
            kdf_or_kser = Series(
                kdf._internal.copy(scol=kdf._internal.column_scols[0]), anchor=kdf
            )
        else:
            kdf_or_kser = kdf
    ...
    **Getting values on a DataFrame with an index that has integer labels**
    ...
    def _get_from_multiindex_column(self, key, labels=None):
        ...
        if all(len(lbl) > 0 and lbl[0] == "" for _, lbl in labels):
            ...
            labels = [
                ...
                for i, (label, lbl) in enumerate(labels)
            ]
    ...
    def _select_cols(self, cols_sel):
        ...
        if cols_sel is None:
            ...
            column_labels = [
                (self._internal.sdf.select(col).columns[0],) for col in cols_sel
            ]

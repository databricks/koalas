    ...
    * `pandas_df` represents pandas DataFrame derived by the metadata
    ...
    def __init__(
        ...
        if index_map is None:
            assert not any(
                SPARK_INDEX_NAME_PATTERN.match(name) for name in sdf.columns
            ), (
        ...
        if scol is not None:
            ...
            self._column_labels = [
                (sdf.select(scol).columns[0],) for scol in self._column_scols
            ]
        else:
    ...
    def scol_for(self, column_labels_or_index_column: Union[str, Tuple[str, ...]]):
        ...
        if column_labels_or_index_column in self._column_labels_to_scol:
            ...
            if column_labels_or_index_column not in self.index_columns:
                raise KeyError(name_like_string(column_labels_or_index_column))
            return scol_for(
                self._sdf, self.column_name_for(column_labels_or_index_column)
            )
    ...
    def spark_type_for(
        ...
        return (
            self._sdf.select(self.scol_for(column_labels_or_index_column))
            .schema[0]
            .dataType
        )
            ...
            pdf = pdf[
                [
                    ...
                    for i, (col, label) in enumerate(
                        zip(self.data_columns, self.column_labels)
                    )
                ]
            ]
    ...
    def with_new_columns(
        ...
        return self.copy(
            ...
            column_scols=[
                scol_for(sdf, col) for col in self._sdf.select(column_scols).columns
            ],
            scol=None,
        )

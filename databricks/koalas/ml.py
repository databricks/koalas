...
def to_numeric_df(
    kdf: "ks.DataFrame",
) -> Tuple[pyspark.sql.DataFrame, List[Tuple[str, ...]]]:
    """
    ...
    numeric_df = kdf._sdf.select(
        *[kdf._internal.scol_for(idx) for idx in numeric_column_labels]
    )
    va = VectorAssembler(
        inputCols=numeric_df.columns, outputCol=CORRELATION_OUTPUT_COLUMN
    )

...
def _build_type_dict():
    return dict(
        [
            (other_type, spark_type)
            for (spark_type, l) in _base.items()
            for other_type in l
        ]
...
def _make_fun(
    f: typing.Callable, return_type: types.DataType, *args, **kwargs
) -> "ks.Series":
    """
    ...
    for arg in args:
        if isinstance(arg, Series):
            ...
            raise ValueError(
                "A pyspark column was passed as an argument."

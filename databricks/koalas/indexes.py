...
class Index(IndexOpsMixin):
    ...
    def identical(self, other):
        ...
        return (self is other) or (
            ...
            and self_name  # to support non-index comparison by short-circuiting.
            == other_name
    ...
    def rename(
        ...
        kdf._internal = kdf._internal.copy(
            index_map=list(zip(kdf._internal.index_columns, names))
        )
    ...
    def to_series(self, name: Union[str, Tuple[str, ...]] = None) -> Series:
        ...
        return Series(
            kdf._internal.copy(
                scol=scol, column_labels=column_labels, column_label_names=None
            ),
            anchor=kdf,
        )
    ...
    def unique(self, level=None):
        ...
        sdf = self._kdf._sdf.select(
            self._scol.alias(self._internal.index_columns[0])
        ).distinct()
        return DataFrame(
            _InternalFrame(sdf=sdf, index_map=self._kdf._internal.index_map)
        ).index
    ...
    def __repr__(self):
        ...
        pindex = (
            ...
            .index._with_new_scol(self._scol)
            .to_pandas()
        )
...
class MultiIndex(Index):
    ...
    def levshape(self):
        ...
        result = internal._sdf.agg(
            *(F.countDistinct(c) for c in internal.index_scols)
        ).collect()[0]
        return tuple(result)

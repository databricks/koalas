...
def range(
    ...
    sdf = default_session().range(
        start=start, end=end, step=step, numPartitions=num_partitions
    )
    ...
    if usecols is None or callable(usecols) or len(usecols) > 0:
        ...
        if usecols is not None:
            if callable(usecols):
                ...
                cols = [
                    field.name for i, field in enumerate(sdf.schema) if i in usecols
                ]
                missing = [
                    col
                    for col in usecols
...
def read_table(
    name: str, index_col: Optional[Union[str, List[str]]] = None
) -> DataFrame:
    """
...
def read_spark_io(
    ...
    **options
    ...
    4  14
    """
    sdf = default_session().read.load(
        path=path, format=format, schema=schema, **options
    )
    index_map = _get_index_map(sdf, index_col)
...
def get_dummies(
    ...
    if isinstance(data, Series):
        ...
        if columns is None:
            column_labels = [
                ...
                if isinstance(
                    kdf._internal.spark_type_for(label),
    ...
    if any(
        not isinstance(
            kdf._internal.spark_type_for(label), _get_dummies_acceptable_types
        )
        for label in column_labels
    ):
        raise NotImplementedError(
    ...
    all_values = _reduce_spark_multi(
        kdf._sdf,
        [F.collect_set(kdf._internal.scol_for(label)) for label in column_labels],
    )
    for i, label in enumerate(column_labels):
        ...
        if dummy_na:
            remaining_columns.append(
                kdf[label].isnull().astype(dtype).rename(column_name("nan"))
            )
...
def concat(objs, axis=0, join="outer", ignore_index=False):
    ...
    if all(name == index_names_of_kdfs[0] for name in index_names_of_kdfs) and all(
        ...
        if join == "inner":
            ...
            merged_columns = sorted(
                list(
                    map(
                        lambda c: column_labelses_of_kdfs[0][
                            column_labelses_of_kdfs[0].index(c)
                        ],
                        interested_columns,
                    )
                )
            ...
            for kdf in objs:
                columns_to_add = list(
                    set(merged_columns) - set(kdf._internal.column_labels)
                )
    ...
    if ignore_index:
        ...
        sdfs = [
            kdf._sdf.select(kdf._internal.index_scols + kdf._internal.column_scols)
            for kdf in kdfs
        ]
    ...
    result_kdf = DataFrame(
        kdfs[0]._internal.copy(
            ...
            index_map=index_map,
            column_scols=[
                scol_for(concatenated, col) for col in kdfs[0]._internal.data_columns
            ],
        )
    )
    ...
    if index_col is not None:
        ...
        for col in index_col:
            if col not in sdf_columns:
                raise KeyError(col)
        index_map = [
            (col, (col,)) for col in index_col
        ]  # type: Optional[List[IndexMap]]
    else:
        index_map = None
